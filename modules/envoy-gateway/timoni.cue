...
import (
	templates "timoni.sh/envoy-gateway/templates"
)
...
timoni: {
	...
	// Pass Kubernetes resources outputted by the instance to Timoni's
	// multi-step apply: in cert-manager TLS mode the certificates are
	// applied and health-checked first, so the certificate secrets
	// exist before the certgen Job and the controller pods start.
	// Both sets are declared unconditionally to fix their order; the
	// certs set is empty in certgen TLS mode and skipped by the
	// applier.
	apply: {
		certs: [for obj in instance.certObjects {obj}]
		app: [for obj in instance.objects {obj}]
	}
}
