...
#NetworkPolicyIngress: netv1.#NetworkPolicy & {
	...
	kind:       "NetworkPolicy"
	metadata: {
		name:      "\(_config.metadata.name)-\(_component)-allow-ingress"
	...
	spec: {
		...
		policyTypes: ["Ingress"]
		ingress: _config.certController.networkPolicy.ingress
	}
}
...
#NetworkPolicyEgress: netv1.#NetworkPolicy & {
	...
	kind:       "NetworkPolicy"
	metadata: {
		name:      "\(_config.metadata.name)-\(_component)-allow-egress"
		namespace: _meta.namespace
		labels:    _meta.labels
		if _meta.annotations != _|_ {annotations: _meta.annotations}
	}
	spec: {
		podSelector: matchLabels: #SelectorLabels & {#config: _config}
		policyTypes: ["Egress"]
		egress: _config.certController.networkPolicy.egress
	}
}
