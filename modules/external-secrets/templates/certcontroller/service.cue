...
// The cert-controller metrics Service, created when
// `certController.metrics.service.enabled` or the ServiceMonitor is set.
#Service: corev1.#Service & {
	#config: config.#Config
	_config: #config
	_svc:    _config.certController.metrics.service

	apiVersion: "v1"
	kind:       "Service"
	metadata: #ClusterObjectMeta & {#config: _config}
	metadata: {
		name:      "\(_config.metadata.name)-cert-controller-metrics"
