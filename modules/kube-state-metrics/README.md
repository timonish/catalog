...
<!-- versions:start -->
Latest module version is `2.19.1-3`, packaging the upstream release
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `serviceMonitor.enabled` | `bool` | `false` | Create a Prometheus Operator ServiceMonitor scraping the metrics endpoint (and the telemetry endpoint with `selfMonitor`), in the instance namespace |
...
| `serviceMonitor.namespaceSelector` | `[...string]` | unset | Namespaces the Service is selected from; empty means the instance namespace |
| `serviceMonitor.http` | endpoint | Prometheus defaults | Scrape settings of the metrics endpoint: `interval`, `scrapeTimeout`, `honorLabels`, `enableHttp2` (default `false`), `scheme`, `tlsConfig`, `bearerTokenFile`, `bearerTokenSecret`, `proxyUrl`, `metricRelabelings`, `relabelings` |
| `serviceMonitor.metrics` | endpoint | Prometheus defaults | Scrape settings of the telemetry endpoint; same fields as `http` |
