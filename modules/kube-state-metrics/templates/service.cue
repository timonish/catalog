...
#Service: corev1.#Service & {
	...
	spec: corev1.#ServiceSpec & {
		...
		if _config.service.clusterIP != _|_ && _config.service.clusterIP != "" {
		...
		ports: [
			...
			if _config.selfMonitor.enabled {
				{
					name:        "metrics"
					port:        _config.selfMonitor.telemetryPort
					protocol:    "TCP"
					targetPort:  "metrics"
					appProtocol: "http"
					if _config.service.type == "NodePort" {
						if _config.service.telemetryNodePort > 0 {
							nodePort: _config.service.telemetryNodePort
						}
					}
				}
			},
