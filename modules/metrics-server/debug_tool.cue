...
// Example 'cue cmd -t debug -t name=metrics-server -t namespace=kube-system -t mv=1.0.0 -t kv=1.31.0 build'.
command: build: {
...
// Example 'cue cmd -t debug -t name=metrics-server -t namespace=kube-system -t mv=1.0.0 -t kv=1.31.0 ls'.
command: ls: {
	task: print: cli.Print & {
